...
.{MARKER} svg{{width:28px;height:28px;display:block;fill:currentColor;flex:0 0 28px}}
...
@media(max-width:760px){{
  ...
  .{MARKER} span{{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}}
}}
...
HTML = f"""
<a class="{MARKER}" data-whatsapp-contact="villa-almale" href="{URLS['fr']}" target="_blank" rel="noopener noreferrer" aria-label="Contacter Villa Almale sur WhatsApp" title="WhatsApp">
...
(() => {{
  ...
  if (!button) return;
  const links = {URLS!r};
  const labels = {{fr:'Contacter Villa Almale sur WhatsApp',en:'Contact Villa Almale on WhatsApp',es:'Contactar con Villa Almale por WhatsApp'}};
  const update = () => {{
